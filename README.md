...
| `OBOLUS_BACKENDS_FILE` | unset | **Multi-backend override.** Path to a JSON file declaring backends — an array of `{"id","kind","baseUrl", …}` objects, where `kind` is `ollama` \| `openai-compat` \| `anthropic-compat`, with an optional `keyFile` (a path to a file holding the bearer token — the key is *referenced*, never inlined) and optional `models` / `precedence` (parsed and carried, but nothing routes on them yet — model routing lands later). When set it **supersedes** `OBOLUS_UPSTREAM_URL`, and setting **both at once is a startup error** (the single-backend value would sit inert). Today exactly one backend may be declared — more than one is a startup refusal until model routing lands. `ollama` is keyless (a `keyFile` on it is refused); `openai-compat` may carry a `keyFile` (a keyless one is allowed, sending no `Authorization`); `anthropic-compat` is refused as not-yet-implemented. A malformed file, an unreadable / empty / non-header-safe key file, a non-`http://` origin, or an empty `id` / model alias is a startup error, not a per-request one. Unset, the single Ollama backend is built from `OBOLUS_UPSTREAM_URL` above. |
